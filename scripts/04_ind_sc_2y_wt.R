# ==================================================
# Individual self-completion two-year weight
# ==================================================
#
# Purpose:
# Create the SCJS two-year individual self-completion weight by combining
# two years of self-completion survey data and calibrating the resulting
# dataset to adult population control totals.
#
# This script:
#   - imports single-year individual self-completion weighting datasets
...
#   - exports final individual self-completion weights
#
# Inputs:
#   - Individual self-completion weighting data for year 1
#   - Individual self-completion weighting data for year 2
#   - Individual self-completion calibration totals
#
# Outputs:
...
#
# Notes:
#   - Age-sex categories are standardised prior to calibration to ensure
#     consistency with the categories used in the population totals file.
#   - Sex values embedded within the age_sex variable are recoded from
#     numeric categories to Male/Female labels where required.
#
# ==================================================
...
message("Execute 2y individual self-completion weight script")
...
y1 <- scjs_import_data(setup$ind_sc_file_y1)
y2 <- scjs_import_data(setup$ind_sc_file_y2)

# import population totals for calibration
totals <- read_csv(config$ind_sc_totals_file,
...
y1_pop <- sum(y1$scjs_sc_ind_wt)
y2_pop <- sum(y2$scjs_sc_ind_wt)
...
names(result$data)[names(result$data)=='preweight.cal'] <- 'SCJS_ind_sc_wt'
result$data$SCJS_ind_sc_wt_sc <- result$data$SCJS_ind_sc_wt * (nrow(combined)/y2_pop)
...
wt_precheck <- scjs_distribution_check(result$data, "SCJS_ind_sc_wt")
...
# Add message to inform user about progress
message("Exporting 2y individual self-completion weights")

indscwts <- result$data %>%
  select(serial, SCJS_ind_sc_wt, SCJS_ind_sc_wt_sc) %>%
  arrange(serial)

write_csv(indscwts, here("output", "SCJSindsc.csv"))
